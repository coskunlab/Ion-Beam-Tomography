...
excel.data1 <- read.xlsx("GTchromatinfor3Dtissue-data1.xlsx", sheetIndex = 1,header=FALSE)
colnames(excel.data1)<-c("Phosx","Phosy","Phosz","Phosval")

excel.data2 <- read.xlsx("GTchromatinfor3Dtissue-data2.xlsx", sheetIndex = 1,header=FALSE)
colnames(excel.data2)<-c("Sulpx","Sulpy","Sulpz","Sulpaval")

excel.data3 <- read.xlsx("GTchromatinfor3Dtissue-data3.xlsx", sheetIndex = 1,header=FALSE)
colnames(excel.data3)<-c("PhoSulx","PhoSuly","Phosulz","PhoSulval")

excel.data4 <- read.xlsx("GTchromatinfor3Dtissue-data4.xlsx", sheetIndex = 1,header=FALSE)
...
row.names(newdata)<-c("Sulphur1","Sulphur2","Sulphur3","Sulphur4","Phos1","Phos2","Phos3","Phos4","X-Spatial","Y-Spatial","Z-Spatial")
...
heatmap.2(newdata,
          ...
          main = " 3D molecular neighborhood map in tissues", # heat map title
