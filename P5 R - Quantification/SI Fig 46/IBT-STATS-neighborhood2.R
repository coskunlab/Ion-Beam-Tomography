...
excel.data1 <- read.xlsx("GTchromatinspatial30-data1.xlsx", sheetIndex = 1,header=FALSE)
colnames(excel.data1)<-c("Tdnax","Tdnay","Tdnaval")

excel.data2 <- read.xlsx("GTchromatinspatial30-data2.xlsx", sheetIndex = 1,header=FALSE)
colnames(excel.data2)<-c("Gdnax","Gdnay","Gdnaval")

excel.data3 <- read.xlsx("GTchromatinspatial30-data3.xlsx", sheetIndex = 1,header=FALSE)
colnames(excel.data3)<-c("Dnax","Dnay","Dnaval")

excel.data4 <- read.xlsx("GTchromatinspatial30-data4.xlsx", sheetIndex = 1,header=FALSE)
...
row.names(newdata)<-c("G-DNA1","G-DNA2","G-DNA3","T-DNA1","T-DNA2","T-DNA3","X-Spatial","Y-Spatial")

par(cex.main=1.6)

heatmap.2(newdata,
          #cellnote = mat,  # same data set for cell labels
          main = "2D molecular neighborhood map in chromatin", # heat map title
