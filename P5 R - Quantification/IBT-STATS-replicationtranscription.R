...
excel.data <- read.xlsx("replicationtranscriptionstats.xlsx", sheetIndex = 1,header=FALSE)

colnames(excel.data)<-c("Replication/DNA","Transcription/Replication")
...
colnames(genesumname)<-c("Colocalization","Overlap")
genesumname<-as.data.frame(genesumname)
genesumname$ReplicationTranscription<-as.factor(genesumname$ReplicationTranscription)
...
p<-ggplot(genesumname, aes(x=Overlap , y=Colocalization,fill=Overlap ))+  geom_boxplot(alpha=0.5)+theme(text = element_text(size=40))+theme(legend.position="none",legend.text = element_text( size=30),legend.title = element_blank())
p + geom_jitter(shape=16, position=position_jitter(0.2),colour="gray48",alpha=0.2)
p+geom_signif(comparisons = list(c("Transcription/Replication","Replication/DNA")), 
              map_signif_level=TRUE,test="wilcox.test",textsize=15)+ ylim(NA, 0.42)+ geom_jitter(shape=16, position=position_jitter(0.2),colour="gray48",alpha=0.5)
