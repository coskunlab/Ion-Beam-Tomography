...
excel.data <- read.xlsx("replicationstats0713.xlsx", sheetIndex = 1,header=FALSE)
...
p+geom_signif(comparisons = list(c("Long", "Short")), 
               map_signif_level=TRUE,test="wilcox.test",textsize=15)+ ylim(NA, 0.55)+geom_jitter(shape=16, position=position_jitter(0.2),colour="gray48",alpha=0.5)
