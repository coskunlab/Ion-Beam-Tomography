...
excel.data <- read.xlsx("TranscriptioninhibitstatsUnnormalized.xlsx", sheetIndex = 1,header=FALSE)
...
p+geom_signif(comparisons = list(c("Normal","Alpha-Amanitin")), 
              map_signif_level=TRUE,test="wilcox.test",textsize=15)+ ylim(0, 17500)+geom_jitter(shape=16, position=position_jitter(0.2),colour="gray48",alpha=0.5)
