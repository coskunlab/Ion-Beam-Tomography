...
data1<-read.delim("C:/Users/ahmet/Desktop/IBT-Files/07132018Idu2hBrduresult-IduAllP.csv", header = TRUE, sep = ",")
data1s<-data1[,4]
namesmtx1<-rep("EarlyReplicatedDNA",each=nrow(data1))



data2<-read.delim("C:/Users/ahmet/Desktop/IBT-Files/07132018Idu2hBrduresult-BrduAllP.csv", header = TRUE, sep = ",")
data2s<-data2[,4]
namesmtx2<-rep("LateReplicatedDNA",each=nrow(data2))

data3<-read.delim("C:/Users/ahmet/Desktop/IBT-Files/07132018Idu2hBrduresult-PhosAllP.csv", header = TRUE, sep = ",")
data3s<-data3[,4]
namesmtx3<-rep("Phoshphorus",each=nrow(data3))

data4<-read.delim("C:/Users/ahmet/Desktop/IBT-Files/07132018Idu2hBrduresult-SulphAllP.csv", header = TRUE, sep = ",")
data4s<-data4[,4]
...
Cellmtx<-eval(Cellmtx[! Cellmtx$Early.Replicated.DNA<1, ])
Cellmtx<-eval(Cellmtx[! Cellmtx$Late.Replicated.DNA<1, ])
Cellmtx<-eval(Cellmtx[! Cellmtx$Phosphorus<15, ])
