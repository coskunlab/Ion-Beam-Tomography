...
data1<-read.delim("C:/Users/ahmet/Desktop/IBT-Files/07132018IduBrrU2hresult-brURnaT.csv", header = TRUE, sep = ",")
data1s<-data1[,4]
namesmtx1<-rep("NewRNA",each=nrow(data1))



data2<-read.delim("C:/Users/ahmet/Desktop/IBT-Files/07132018IduBrrU2hresult-IduT.csv", header = TRUE, sep = ",")
data2s<-data2[,4]
namesmtx2<-rep("ReplicatedDNA",each=nrow(data2))

data3<-read.delim("C:/Users/ahmet/Desktop/IBT-Files/07132018IduBrrU2hresult-phosT.csv", header = TRUE, sep = ",")
data3s<-data3[,4]
namesmtx3<-rep("Phoshphorus",each=nrow(data3))

data4<-read.delim("C:/Users/ahmet/Desktop/IBT-Files/07132018IduBrrU2hresult-sulphurT.csv", header = TRUE, sep = ",")
data4s<-data4[,4]
...
Cellmtx2 <-as.matrix(cbind(data1s,data3s))



colnames(Cellmtx)<-c("New.RNA","Replicated.DNA","Phosphorus","Proteins")
colnames(Cellmtx2)<-c("New.RNA","Replicated.DNA")
...
Cellmtx<-eval(Cellmtx[! Cellmtx$Replicated.DNA<3, ])
Cellmtx<-eval(Cellmtx[! Cellmtx$New.RNA<1, ])
Cellmtx<-eval(Cellmtx[Cellmtx$Phosphorus>20, ])
...
par(cex = 2)
corrplot(m, method="color",tl.col = "black",cl.offset="-0.6",cl.align.text='r')
...
# library(Rtsne)
# train<- Cellmtx
# tsne_data <- tsne(Cellmtx, k=5, epoch_callback=epc, max_iter=500, epoch=100)
...
circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
  ...
  circos.text(mean(xlim), mean(ylim), sector.index, facing = "bending", niceFacing = TRUE, cex=1.4)
}, bg.border = NA)
